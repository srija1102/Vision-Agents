...
A real-time multimodal AI agent that sits inside a live video interview and simultaneously analyzes both participants — measuring candidate stress, interviewer aggression, and conversation dynamics — then streams the results to a live browser dashboard with coaching feedback, spike alerts, session history, and a full post-session performance summary.
...
- **Candidate video** — YOLOv11 pose estimation at 15 fps tracks body posture, shoulder tension, and movement patterns
- **Candidate audio** — Deepgram STT with eager turn detection transcribes speech in real-time and measures filler words, speech rate, pitch variance, and pause patterns
- **Conversation turns** — InterviewBridge tracks who is speaking, interruption patterns, and question cadence
...
1. **Deterministic scorer** — weighted sum of all signals (weights sum to 1.0 per dimension). A multi-signal gate caps stress at 50% and aggression at 60% if fewer than 2 signals are active, preventing single-feature false positives.
2. **LLM refiner** — Claude Haiku receives the pre-computed scores as anchors and can adjust them within ±0.15 bounds, then generates the coaching tip. The LLM never produces scores from scratch — it can only refine.
...
| Panel | What it shows |
|-------|---------------|
| **Candidate Stress** | 0–100% stress score with confidence level |
| **Interviewer Aggression** | 0–100% aggression score with confidence level |
...
| **Spike Log** | Detected stress spikes with σ magnitude and cause |
| **Session Metrics** | Interviewer style badge, dominant party, imbalance score |
| **Conversation Transcript** | Live Q&A feed — every candidate utterance and AI response |
...
| **📁 History** | Button opens a modal listing every past session — click any to replay its full data into the dashboard |
...
- Every spike event with cause and σ magnitude
...
**Auto-summary on leave** — the moment the candidate closes the interview tab, the full session summary is pushed to the dashboard immediately. No need to wait for the call to formally end.

**Session comparison** — on the second run with the same call-id, the summary overlay shows per-metric delta percentages vs the previous session (e.g. "Stress ↓ 14%", "Resilience ↑ 22%").

**Graceful degradation** — if STT, video, or the LLM is unavailable, the system falls back to the deterministic result rather than failing.

**Skip-not-queue** — if the previous analysis cycle is still running when the next one is due, it is skipped rather than queued. This prevents a backlog of stale analysis windows.
...
### Option 1 — Dashboard Preview (no credentials needed)
...
- The Stream join link — where the candidate connects
...
```
Candidate video  → YOLOv11 pose (15 fps) → PoseBridge  ──┐
Candidate audio  → Deepgram STT          → InterviewBridge─┤→ BehavioralAnalyzer
Turn events      → InterviewBridge       ────────────────┘

Every 5 seconds:
  BehavioralAnalyzer
    → deterministic weighted scorer (11 stress + 5 aggression signals)
    → 60s per-session z-score baseline normalizer
    → multi-signal gate (prevents false positives)
    → Claude Haiku anchored refiner (adjusts ±0.15, generates coaching tip)
    → BehavioralAnalysisEvent emitted on agent event bus

  BehavioralAnalysisEvent
    → WebSocket → Browser dashboard
    → SQLite (sessions/interviews.db)
    → Terminal log

On participant leave / call end:
  SpikeDetector + ResilienceTracker + SessionSummaryGenerator
    → SessionSummaryEvent
    → WebSocket → Summary overlay
    → SQLite (summaries table)
    → sessions/<id>_<timestamp>.json (backup)
```
...
| Module | Role |
|--------|------|
| `interview_analyzer.py` | Main entry point — wires the agent, dashboard, and all processors |
| `preview_dashboard.py` | Synthetic replay — demos the dashboard without a live call |
...
| `_spike_detector.py` | Detects >2σ stress spikes, tags cause (question / interruption / silence) |
...
| Combine YOLO + LLM in real-time | YOLOv11 pose → BehavioralAnalyzer → Claude Haiku, every 5 seconds |
